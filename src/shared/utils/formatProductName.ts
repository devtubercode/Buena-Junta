function lowerFirst(value: string) {
  return value.length > 0
    ? `${value[0].toLocaleLowerCase("es-CO")}${value.slice(1)}`
    : value;
}
...
export function formatProductName(name: string) {
  ...
  if (flavorMatch) {
    ...
    const singularProduct = productType
  ...
  const pizzaMatch = name.match(
    /^(.+)\s*\((Personal|Pequeña|Pequena|Mediana|Grande)\s*x\d+\)$/i,
  );
