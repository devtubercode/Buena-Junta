...
import type { CategoryInput, CategoryRow } from "@/features/admin/types/admin.types";
...
export async function saveCategory(input: CategoryInput, id?: string) {
  const result = id
    ? await supabase.from(SUPABASE_TABLES.CATEGORIES).update(input).eq("id", id)
    : await supabase.from(SUPABASE_TABLES.CATEGORIES).insert(input);
